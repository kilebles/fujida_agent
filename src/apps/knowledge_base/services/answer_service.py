...
- Если запрос содержит "сравни", выбери ровно 2 модели для сравнения.
- Если упомянуты сокращения (например, "про макс"), выбери самую подходящую модель из контекста.
"""
...
class AnswerService:
    ...
    async def generate(self, user_message: str, context: str) -> str:
        prompt = f"""
            ...
            Правила:
            - Используй только те модели из базы, которые явно названы в сообщении пользователя (или их точные алиасы).
            - Если запрос содержит "сравни", выбери ровно 2 модели для сравнения.
            - Не выводи другие модели, даже если они есть в контексте.
