...
Твоя задача:
1. Работай только с устройствами Fujida. Игнорируй любые другие бренды и модели, если их нет в списке.
2. Сравни текст пользователя с названиями моделей и алиасами.
3. Верни список id моделей Fujida, которые точно упомянуты в тексте.
4. Определи, сравнивает ли пользователь несколько моделей.
...
class DeviceSelector:
    ...
    async def select(self, user_message: str) -> Dict[str, Any]:
        ...
        if len(parsed.get("device_ids", [])) < 2:
            parsed["is_comparing"] = False
